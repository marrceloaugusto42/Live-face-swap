...
export default function App(){
 ...
 function upload(e:React.ChangeEvent<HTMLInputElement>){const f=e.target.files?.[0];if(!f)return;const url=URL.createObjectURL(f);setSourceUrl(url);sourcePoints.current=null;swapTriangles=null;if(source.current){source.current.onload=()=>{if(!analyzeSource())setStatus(landmarker.current?"No face detected in source image — choose a clear front-facing photo.":"Source loaded — face tracker is still loading…");};source.current.src=url}setStatus(running?"Analyzing source face…":"Source loaded — start the camera.")}
